...
const Grass = function (mapObject) {
    this.type = 'grass';
    this.xPos = mapObject.xPos;
    this.yPos = mapObject.yPos;
...
const map = {
    ...
    show: function () {
        ...
        for (let i = 0; i < this.MAP_HEIGHT; i++) {
            for (let j = 0; j < this.MAP_WIDTH; j++) {
                const energy = this.COORDS_LIST[i][j].energy;
                const energyСoef = this.COORDS_LIST[i][j].energyKoef;
                const energySize = '';
    ...
    setObjectToMap: function (mapObject) {
        if (!mapObject.xPos || !mapObject.yPos) {
            const allEmptyCellsArr = this.findFromAllEmptyCells();
            if (!allEmptyCellsArr) {
                map.stop();
    ...
    deleteObject: function () {
        const clickedPositionList = $(this).closest('.map-frame').data('pos').split(',');
    ...
    moveObject: function (mapObject) {
        const randomObjValues = this.getRandomEmptyPlace(mapObject);
        const newObj = {};
    ...
    getRandomEmptyPlace: function (mapObject) {
        const emptyCellsList = this.findEmptyCells(mapObject);
        if (!emptyCellsList) {
            return {xPos: mapObject.xPos, yPos: mapObject.yPos, energy: mapObject.energy - mapObject.moveStepEnergy}
        }
        const randomEmptyCell = this.randomUseKoef(emptyCellsList);
        const energy = Math.floor(Math.random() * (mapObject.maxRandomEnergy - mapObject.minRandomEnergy + 1) + mapObject.minRandomEnergy);
    ...
    findEmptyCells: function (mapObject) {
        ...
        //умная корова
        const filterGrassList = emptyCellsList.filter(this.filterGrass);
...
const mapStop = function () {
    $.proxy(map.stop(), map);
};

const setNewTimeInterval = function () {
